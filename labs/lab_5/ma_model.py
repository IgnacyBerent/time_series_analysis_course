...
def ma_series(burnin: int, n: int, c: float, O: np.array, seed: int=None) -> tuple[np.array, str]:
    ...
    -------
    tuple[np.array, str]
        The generated moving average series and the formula used to generate it.
...
def ma_series_linear(burnin: int, n: int, c: float, O: np.array, seed: int=None) -> tuple[np.array, str]:
    """
    Generate a moving average series using linear algebra approach.
    ...
    for t in range(q, n+burnin):
        y[t] = c + e[t] + np.dot(O[::-1], e[t-q:t])

    formula = f"MA model: y[t] = {c} {' '.join([f'{O[i]:+}y[t-{i+1}]' for i in range(q)])} + e[t]"
    return y[burnin:], formula